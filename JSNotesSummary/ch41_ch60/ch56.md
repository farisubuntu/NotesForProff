...
//In the following code, the function inner closes
// over both fn and args.
function curry(fn) {
  const args = [];
...
```javascript
// In the following example, all the implementation details
// are hidden inside an immediately executed function
// expression. The functions tick and toString close over
// the private state and functions they need to complete
// their work.
let namespace = {};
...
- **Hoisting**: before a function is executed, its body is parser/scanned for declarations (i.e., `var` & `function declaration`)
  ...
  – Resulting local allocations are created in a local scope object before any code in the function body is run
    - `var` declarations create a local scope variable but do not assign a value.
